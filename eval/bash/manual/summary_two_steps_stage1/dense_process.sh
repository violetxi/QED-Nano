...
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage1-qwen3-4b-dense-process-24k \
  --data-path lm-provers/ProofBench \
  --output-path outputs/stage1-qwen3-4b-dense-process-proofbench-n128-summary-24k.jsonl \
  --overwrite \
  --n 128 \
  --summary-max-tokens 16384
...
uv run python scripts/eval.py \
  --model-config openai/gpt-5-nano \
  --data-path outputs/stage1-qwen3-4b-dense-process-proofbench-n128-summary-24k.jsonl \
  --output-path outputs/stage1-qwen3-4b-dense-process-proofbench-n128-summary-24k-graded.jsonl \
  --proofbench

# 5f: ProofBench stats (n128, 24k)
uv run python scripts/stats.py outputs/stage1-qwen3-4b-dense-process-proofbench-n128-summary-24k-graded.jsonl
