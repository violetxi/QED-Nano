#!/bin/bash
# === Manual two-step eval (generate + summarize, grade reasoning trace): instruct_raw ===
...
uv run python scripts/eval.py \
  ...
  --output-path outputs/stage2-qwen3-4b-instruct-raw-imoproofbench-summary-reasoning-graded.jsonl \
  --submission-column reasoning_trace

# 2c: IMOProofBench stats
uv run python scripts/stats.py outputs/stage2-qwen3-4b-instruct-raw-imoproofbench-summary-reasoning-graded.jsonl
...
uv run python scripts/eval.py \
  ...
  --output-path outputs/stage2-qwen3-4b-instruct-raw-proofbench-summary-reasoning-graded.jsonl \
  --submission-column reasoning_trace \
  --proofbench

# 2f: ProofBench stats
uv run python scripts/stats.py outputs/stage2-qwen3-4b-instruct-raw-proofbench-summary-reasoning-graded.jsonl
