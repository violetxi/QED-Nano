#!/bin/bash
# === Manual two-step eval (generate + summarize): dense_outcome ===
...
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-outcome-imoproofbench-summary-graded.jsonl
...
# 2f: ProofBench stats
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-outcome-proofbench-summary-graded.jsonl
...
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-outcome-24k \
  --output-path outputs/stage2-qwen3-4b-dense-outcome-imoproofbench-summary-24k.jsonl \
  --overwrite \
  --n 16 \
...
uv run python scripts/eval.py \
  --model-config google/gemini-3-pro \
  --data-path outputs/stage2-qwen3-4b-dense-outcome-imoproofbench-summary-24k.jsonl \
  --output-path outputs/stage2-qwen3-4b-dense-outcome-imoproofbench-summary-24k-graded.jsonl

# 3c: IMOProofBench stats (24k)
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-outcome-imoproofbench-summary-24k-graded.jsonl

# 3d: ProofBench (generate + summarize, 24k)
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-outcome-24k \
  --data-path lm-provers/ProofBench \
  --output-path outputs/stage2-qwen3-4b-dense-outcome-proofbench-summary-24k.jsonl \
  --overwrite \
  --n 16 \
  --summary-max-tokens 16384
...
uv run python scripts/eval.py \
  --model-config google/gemini-3-pro \
  --data-path outputs/stage2-qwen3-4b-dense-outcome-proofbench-summary-24k.jsonl \
  --output-path outputs/stage2-qwen3-4b-dense-outcome-proofbench-summary-24k-graded.jsonl \
  --proofbench

# 3f: ProofBench stats (24k)
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-outcome-proofbench-summary-24k-graded.jsonl
