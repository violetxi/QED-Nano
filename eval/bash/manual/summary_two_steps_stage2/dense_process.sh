#!/bin/bash
# === Manual two-step eval (generate + summarize): dense_process ===
...
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-process \
  --output-path outputs/stage2-qwen3-4b-dense-process-imoproofbench-summary.jsonl \
  --overwrite \
  --n 16
...
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-process-imoproofbench-summary-graded.jsonl

# 2d: ProofBench (generate + summarize)
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-process \
  --data-path lm-provers/ProofBench \
  --output-path outputs/stage2-qwen3-4b-dense-process-proofbench-summary.jsonl \
  --overwrite \
  --n 16
...
# 2f: ProofBench stats
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-process-proofbench-summary-graded.jsonl
...
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-process-24k \
  --output-path outputs/stage2-qwen3-4b-dense-process-imoproofbench-summary-24k.jsonl \
  --overwrite \
  --n 16 \
...
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-process-imoproofbench-summary-24k-graded.jsonl

# 3d: ProofBench (generate + summarize, 24k)
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-process-24k \
  --data-path lm-provers/ProofBench \
  --output-path outputs/stage2-qwen3-4b-dense-process-proofbench-summary-24k.jsonl \
  --overwrite \
  --n 16 \
  --summary-max-tokens 16384
...
uv run python scripts/stats.py outputs/stage2-qwen3-4b-dense-process-proofbench-summary-24k-graded.jsonl

# Step 4: Kill the vLLM server (Ctrl+C in pane 1, or:)
# pkill -f "vllm.entrypoints.openai.api_server --model violetxi/exp_stage2_proof_qwen3-4b_dense_process"
