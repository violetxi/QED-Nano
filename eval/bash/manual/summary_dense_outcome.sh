#!/bin/bash
# === Manual summary eval: dense_outcome ===
...
uv run python scripts/run_summary.py \
  ...
  --generation-path outputs/stage2-qwen3-4b-dense-outcome-imoproofbench.jsonl \
  --output-path outputs/stage2-qwen3-4b-dense-outcome-imoproofbench-summary.jsonl

# 2b: Summarize ProofBench
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-outcome \
  --generation-path outputs/stage2-qwen3-4b-dense-outcome-proofbench.jsonl \
