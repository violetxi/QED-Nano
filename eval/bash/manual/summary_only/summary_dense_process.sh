#!/bin/bash
# === Manual summary eval: dense_process ===
...
uv run python scripts/run_summary.py \
  ...
  --generation-path outputs/stage2-qwen3-4b-dense-process-imoproofbench.jsonl \
  --output-path outputs/stage2-qwen3-4b-dense-process-imoproofbench-summary.jsonl

# 2b: Summarize ProofBench
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-dense-process \
  --generation-path outputs/stage2-qwen3-4b-dense-process-proofbench.jsonl \
