#!/bin/bash
# === Manual summary eval: self_distill ===
...
uv run python scripts/run_summary.py \
  ...
  --generation-path outputs/stage2-qwen3-4b-self-distill-imoproofbench.jsonl \
  --output-path outputs/stage2-qwen3-4b-self-distill-imoproofbench-summary.jsonl

# 2b: Summarize ProofBench
uv run python scripts/run_summary.py \
  --model-config vllm/vllm-violetxi-stage2-qwen3-4b-self-distill \
  --generation-path outputs/stage2-qwen3-4b-self-distill-proofbench.jsonl \
